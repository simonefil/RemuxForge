...
namespace RemuxForge.Core.Analysis.Features
{
    ...
    public abstract class FrameFeatureBatchMatcherBase : IDisposable
    {
        ...
        public static FrameFeatureBatchMatcherBase Create(VisionBackendKind backend)
        {
            return Create(backend, null);
        }

        /// <summary>
        /// Crea il matcher richiesto con opzioni condivise esplicite
        /// </summary>
        /// <param name="backend">Backend SIFT operativo</param>
        /// <param name="options">Opzioni comuni CPU/Vulkan, oppure null per i valori predefiniti</param>
        /// <returns>Matcher del backend richiesto</returns>
        public static FrameFeatureBatchMatcherBase Create(VisionBackendKind backend, FrameFeatureMatcherOptions options)
        {
            switch (backend)
            {
                case VisionBackendKind.Cpu:
                    return new OpenCvSiftBatchMatcher(options);
                case VisionBackendKind.Vulkan:
                    return new VulkanSiftBatchMatcher(options);
                default:
        ...
        /// <summary>
        /// Converte una cella accettata e la relativa omografia in una diagnostica temporale
        /// </summary>
        ...
        /// <param name="homography">Omografia source-language, oppure null</param>
        protected void AddAcceptedPair(DeepSiftBatchMatchResult batch, int sourceIndex, int languageIndex, DeepSiftMatchCell cell, double[] homography)
        {
