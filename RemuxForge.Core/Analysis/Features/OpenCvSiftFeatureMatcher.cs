...
namespace RemuxForge.Core.Analysis.Features
{
    ...
    public sealed class OpenCvSiftFeatureMatcher : IDisposable
    {
        ...
        /// <summary>
        /// Protegge il solo RNG globale usato dal RANSAC classico di OpenCV
        /// </summary>
        private static readonly object s_ransacRngLock = new object();
        ...
        private FrameFeatureMatchResult CompleteGeometricMatch(FrameFeatureMatchResult result, OpenCvSiftFeatureSet source, OpenCvSiftFeatureSet language, List<DMatch> reciprocalMatches, int forwardRatioMatchCount, int randomSeed)
        {
            ...
            {
                Mat homography;
                lock (s_ransacRngLock)
                {
                    Cv2.SetTheRNG(unchecked((uint)randomSeed));
                    homography = Cv2.FindHomography(sourcePoints, languagePoints, HomographyMethods.Ransac, this._options.RansacReprojectionThreshold, inlierMask);
                }
                using (homography)
                {
